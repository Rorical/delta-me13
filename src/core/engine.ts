...
export class WorldEngine {
  ...
  onEraEnd?: (outcome: 'recreation' | 'collapse') => void;
  ...
  attackPower(a: AgentStatus): number {
    ...
    const lvl = isHeir(a) ? (a.level - 1) * 2 + a.embers.length * 3 : 0;
    return a.power + w + lvl;
  }

  defensePower(a: AgentStatus): number {
    const arm = a.armor ? getItem(a.armor)?.defense ?? 0 : 0;
    const lvl = isHeir(a) ? a.level - 1 : 0;
    return a.defense + arm + lvl;
  }
  ...
  private dispatch(actor: AgentStatus, action: Action): ActionResult {
    ...
    switch (action.type) {
      case 'MOVE': {
        ...
        const hop = nextHop(this.state.cities, actor.location, dest.id);
        ...
        this.log('move', `${actor.name} 从${from}来到${hop}${tail}`, 'low', { agentId: actor.id, location: hop });
        return { ok: true, message: `抵达${hop}${tail}` };
      }
      ...
      case 'INSPECT': {
        const t = this.sameCityTarget(actor, action.targetId);
        if (typeof t === 'string') return { ok: false, message: t };
        const extra = isHeir(t) ? `火种${t.embers.length}枚，等级${t.level}` : isTitan(t) ? `火种${t.emberTaken ? '已失' : '仍在'}，对你的认可${t.respect[actor.id] ?? 0}` : `身份${t.subtitle}`;
  ...
  private combat(attacker: AgentStatus, target: AgentStatus): ActionResult {
    ...
    // 同城的盟友协同作战
    const helpers = attacker.allies
      .map(id => this.agent(id))
      .filter((a): a is AgentStatus => !!a && a.location === attacker.location && a.condition === 'active'
        && a.id !== target.id && !a.allies.includes(target.id))
      .sort((x, y) => this.attackPower(y) - this.attackPower(x))
      .slice(0, 3);
    const assist = helpers.reduce((sum, h) => sum + this.attackPower(h) * 0.5, 0);
    const roll = 0.85 + Math.random() * 0.3;
    let dmg = Math.round((this.attackPower(attacker) + assist) * roll - this.defensePower(target) * 0.5);
    if (target.guarding) dmg = Math.round(dmg / 2);
    dmg = Math.max(1, dmg);
    target.hp -= dmg;
    ...
    const verb = isTitan(target) ? '挑战' : '攻击';
    this.log('combat', `${attacker.name}${withHelp}${verb}${target.name}，造成${dmg}点伤害（剩余${Math.max(0, target.hp)}/${target.maxHp}）`,
      isTitan(target) ? 'high' : 'medium', { agentId: attacker.id, targetId: target.id, location: city?.id });

    if (isHeir(attacker)) this.gainXp(attacker, isTitan(target) ? 6 : 3);
    ...
    // 泰坦与战士会立即反击
    if (isTitan(target) || isHeir(target) || (isNpc(target) && target.role === 'GUARD')) {
      let back = Math.round(this.attackPower(target) * (0.8 + Math.random() * 0.3) - this.defensePower(attacker) * 0.5);
      if (attacker.guarding) back = Math.round(back / 2);
      back = Math.max(1, back);
      attacker.hp -= back;
  ...
  private defeat(loser: AgentStatus, winner: AgentStatus) {
    ...
    loser.reviveDay = this.state.day + 2;
    if (isHeir(loser)) {
      loser.deaths++;
      // 倒下时火种不会遗失，但会被黑潮削弱意志
    }
    this.log('combat', `${loser.name} 被${winner.name}击倒，将在两日后于${SAFE_HAVEN}苏醒`, 'high', { agentId: winner.id, targetId: loser.id, location: loser.location });
  }
  ...
  beginDay() {
    ...
    this.log('system', `—— 第${s.era}纪元 · 第${s.day}天 ——`, 'high');
  }
  ...
  private tickDarkTide() {
    ...
    for (const c of cities) {
      ...
      // 身处重度黑潮中的居民会受伤
      if (c.darkTide > 70) {
        for (const a of this.agentsIn(c.id)) if (!isTitan(a)) a.hp = Math.max(1, a.hp - 3);
      }
    }
  ...
  private tickRegen() {
    ...
    for (const a of Object.values(this.state.agents)) {
      if (a.condition !== 'active') continue;
      const rate = isTitan(a) ? 0.02 : 0.03;
  ...
  private rollWorldEvent() {
    ...
    if (roll < 0.4) {
      ...
      for (const a of this.agentsIn(city.id)) a.hp = Math.max(1, a.hp - 10);
